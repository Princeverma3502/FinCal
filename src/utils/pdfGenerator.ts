...
import { formatCurrency, CurrencyCode } from './finance';
...
export const generatePDF = (inputs: LoanInputs, results: LoanResults, currency: CurrencyCode) => {
  ...
  doc.addImage('/icon-512.png', 'PNG', 15, 10, 20, 20); 

  doc.setTextColor(255, 255, 255);
  doc.setFontSize(22);
  doc.setFont('helvetica', 'bold');
  doc.text('FinCal Mortgage Report', 40, 25); 
  ...
  // 3. Yearly Amortization Schedule
  ...
  doc.text('This report is for estimation purposes only. Consult your bank for exact figures.', 15, footerY);
